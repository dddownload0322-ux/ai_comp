...
from nibble_nn import NibbleMLP, file_to_nibble_tokens, make_context_batch, resolve_device, save_checkpoint


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a nibble-level neural probability model.")
    parser.add_argument("--input", required=True, help="training file path")
    parser.add_argument("--model-out", default="nibble_model.pt", help="checkpoint output path")
...
def main() -> None:
    ...
    model = NibbleMLP(context_length=args.context).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr)
    max_start = token_count - args.context

    print(f"input={input_path}")
    print(f"bytes={input_path.stat().st_size if args.max_bytes is None else min(args.max_bytes, input_path.stat().st_size)}")
    print(f"tokens={token_count} context={args.context} device={device}")
    print(f"model: {args.context} -> 16 -> 16 -> 16 -> 16")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step in range(1, args.steps_per_epoch + 1):
            ...
            contexts, targets = make_context_batch(tokens, starts, args.context, device)

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Infer four independent bit probabilities per nibble token.")
    parser.add_argument("--input", required=True, help="file to infer")
    parser.add_argument("--model", required=True, help="trained bit checkpoint path")
    parser.add_argument("--batch-size", type=int, default=1048576)
    parser.add_argument("--device", default="auto", help="auto, cpu, cuda, cuda:0, ...")
    parser.add_argument("--prob-out", default=None, help="optional .npy output for per-bit probabilities")
...
def main() -> None:
    ...
    with torch.inference_mode():
        for first_target in range(context, token_count, args.batch_size):
            ...
            loss = F.binary_cross_entropy_with_logits(logits, bit_targets, reduction="sum")
            ...
            if out_array is not None:
                ...
                if args.prob_format == "freq-u16":
                    out_array[row_start:row_stop] = logits_to_bit_frequencies(logits, args.freq_total)
                else:
    ...
    print(f"binary_cross_entropy={bpt:.4f} bits/token, ideal_compressed/original={theoretical_ratio:.4f}")
    if args.prob_out is not None:

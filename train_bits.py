...
from bit_nibble_nn import BitNibbleMLP, file_to_nibble_tokens, make_bit_context_batch, resolve_device, save_bit_checkpoint


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a nibble-context model that predicts four independent bits.")
    parser.add_argument("--input", required=True, help="training file path")
    parser.add_argument("--model-out", default="bit_model.pt", help="checkpoint output path")
...
def main() -> None:
    ...
    model = BitNibbleMLP(context_length=args.context).to(device)
    ...
    print(f"model: {args.context} -> 16 -> 16 -> 16 -> 4")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step in range(1, args.steps_per_epoch + 1):
            ...
            contexts, bit_targets = make_bit_context_batch(tokens, starts, args.context, device)

            optimizer.zero_grad(set_to_none=True)
            logits = model(contexts)
            loss = F.binary_cross_entropy_with_logits(logits, bit_targets, reduction="none").sum(dim=1).mean()
            loss.backward()
            optimizer.step()
